...
public class WordSuggesterHelper : MonoBehaviour
{
    ...
    async public void Populate(String wordInEnglish, Languages targetLanguage, TextMeshProUGUI text1, TextMeshProUGUI text2, TextMeshProUGUI text3)
    {
        ...
        {
            text1.text = AppManager.Instance.CapitalizeFirstLetter(relatedWords[0].Item2).Split()[0];
        }
        if (relatedWords.Count >= 2)
        {
            text2.text = AppManager.Instance.CapitalizeFirstLetter(relatedWords[1].Item2).Split()[0];
        }
        if (relatedWords.Count >= 3)
        {
            text3.text = AppManager.Instance.CapitalizeFirstLetter(relatedWords[2].Item2).Split()[0];
        }
    }
}
